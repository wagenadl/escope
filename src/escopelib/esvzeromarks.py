        ...
        for k in range(self.cfg.MAXCHANNELS):
            if np.isnan(self.cfg.conn.hw[k]):
                ...
                if yp<0:
                    ...
                    # Regular marker
                    p.drawEllipse(QPoint((xlp+xrp)/2,yp),rsclp,rsclp)
                    p.drawLine(xrp,yp,wp,yp)
                    self.yp[k]=yp

    def wheelEvent(self,evt):
        if self.wheeling==None:
            evt.ignore()
            return
        ...
        if delta!=0:
            ...
            self.update()
            print 'Offset %i changed to %.1f divisions' % (k, y)
            self.cfgChanged.emit(k)

    def mousePressEvent(self,evt):
        x=evt.x()
        y=evt.y()
        k = np.argmin((self.xp-x)**2+(self.yp-y)**2)
